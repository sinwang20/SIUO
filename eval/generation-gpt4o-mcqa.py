...
logging.basicConfig(filename='./gpt-call/gpt4o-mcqa/log_file.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
    ...
    for idx in range(start_idx, end_idx + 1):
        ...
        if item['question_id'] not in forbidden_list:
            ...
            try:
                ...
                prompt_template = "Answer the following multi-choice questions with the option's letter directly.\n\nQusetion: {question}\nOption:\n{option}\n\nChocie:"
                
                prompt = prompt_template.replace("{question}", question).replace("{option}", options)

                response = api_call(prompt, base64_image)
                
                resp = unicodedata.normalize('NFKC', response.choices[0].message.content)
                item["response_mcqa"] = resp


                # Save each item's result in a separate JSON file
                with open(f'./gpt-call/gpt4o-mcqa/output_{idx}-{question_id}.json', 'w', encoding='utf-8') as outfile:
                    json.dump(item, outfile, ensure_ascii=False, indent=4)
            except Exception as e:
                # Log the error
                with open('./gpt-call/gpt4o-mcqa/error_log.txt', 'a') as log_file:
...
def main():
    data = load_data('./data/siuo_mcqa.json')
